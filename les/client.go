...
import (
	...
	lpc "github.com/ionchain/ionchain-core/les/lespay/client"
...
func New(stack *node.Node, config *ionc.Config) (*LightIonchain, error) {
	...
	leth := &LightIonchain{
		...
		engine:         ionc.CreateConsensusEngine(stack, chainConfig, &config.Ethash, nil, false, chainDb),
		bloomRequests:  make(chan chan *bloombits.Retrieval),
		bloomIndexer:   ionc.NewBloomIndexer(chainDb, params.BloomBitsBlocksClient, params.HelperTrieConfirmations),
	...
	leth.netRPCService = ioncapi.NewPublicNetAPI(leth.p2pServer, leth.config.NetworkId)
...
func (s *LightIonchain) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightIonchain) Downloader() *downloader.Downloader { return s.handler.downloader }
...
// light ionchain protocol implementation.
func (s *LightIonchain) Start() error {
...
func (s *LightIonchain) Stop() error {
	...
	s.chainDb.Close()
	s.wg.Wait()
	log.Info("Light ionchain stopped")
	return nil
}
