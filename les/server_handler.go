...
import (
	...
	lps "github.com/ionchain/ionchain-core/les/lespay/server"
...
func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb ioncdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light IonChain peer rejected", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light ionchain response", "err", err)
			return err
		default:
		}
