...
// the ionc.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb ioncdb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
...
//
// Note, we rely on the underlying ionc/fetcher to retrieve and validate the
// response, so that we have to obey the rule of ionc/fetcher which only accepts
