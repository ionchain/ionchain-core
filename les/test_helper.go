...
import (
	...
	"github.com/ionchain/ionchain-core/contracts/checkpointoracle/contract"
	...
	"github.com/ionchain/ionchain-core/les/checkpointoracle"
	...
	"github.com/ionchain/ionchain-core/p2p/nodestate"
...
func testIndexers(db ioncdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
	var indexers [3]*core.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms, disablePruning)
	indexers[1] = ionc.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db ioncdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
	...
	client := &LightIonchain{
		lesCommons: lesCommons{
			genesis:     genesis.Hash(),
			config:      &ionc.Config{LightPeers: 100, NetworkId: NetworkId},
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db ioncdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
