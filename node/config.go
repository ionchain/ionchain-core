...
import (
	...
	"github.com/ionchain/ionchain-core/accounts/external"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of ionc is "ionc". If no
...
func (c *Config) NodeName() string {
	...
	if name == "ionc" || name == "ionc-testnet" {
		name = "ionc"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "ionc" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by ionc 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'ionc' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-ionchain-keystore")
		ephemeral = keydir
	}
