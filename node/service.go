...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (ioncdb.Database, error) {
	if ctx.config.DataDir == "" {
		return ioncdb.NewMemDatabase()
	}
	db, err := ioncdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
