...
type Downloader struct {
	...
	stateDB    ioncdb.Database // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [ionc/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [ionc/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [ionc/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [ionc/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [ionc/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [ionc/62] Channel to feed the header processor new tasks
	...
	stateCh        chan dataPack // [ionc/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb ioncdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "ionc", p.version, "head", hash, "td", td, "mode", mode)
...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *types.Header) (uint64, error) {
	...
	go p.peer.RequestHeadersByNumber(uint64(from), 1, 0, false)//TODO
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	getHeaders := func(from uint64) {
		...
		if skeleton {
			...
			go p.peer.RequestHeadersByNumber(from, 1, 0, false)//TODO
		} else {
