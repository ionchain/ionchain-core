...
const (
	// bloomServiceThreads is the number of goroutines used globally by an ionchain
...
func (eth *IONChain) startBloomHandlers() {
...
// for the ionchain header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  ioncdb.Database      // database instance to write index data and metadata into
...
func NewBloomIndexer(db ioncdb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := ioncdb.NewTable(db, string(core.BloomBitsIndexPrefix))
