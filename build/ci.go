...
import (
	...
	"github.com/ionchain/ionchain-core/crypto/signify"
	"github.com/ionchain/ionchain-core/internal/build"
	"github.com/ionchain/ionchain-core/params"
)

var (
	// Files that end up in the ionc*.zip archive.
	ioncArchiveFiles = []string{
		"COPYING",
		executablePath("ionc"),
	}

	// Files that end up in the ionc-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert IonChain contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "IonChain bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (IonChain Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "ionc",
			Description: "IonChain CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "IonChain private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "IonChain account management tool.",
		},
	}

	// A debian package is created for all executables listed here.

	debIonchain = debPackage{
		Name:        "ethereum",
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 13 {
			...
			log.Println("go-ionchain requires at least Go version 1.13 and cannot")
...
func doArchive(cmdline []string) {
	...
	var (
		...
		baseionc = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		ionc     = "ionc-" + baseionc + ext
		alltools = "ionc-alltools-" + baseionc + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(ionc, ioncArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{ionc, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with ionc-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "ionc-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-ionchain sources.
...
func downloadGo(goarch, goos, cachedir string) string {
	...
	godir := filepath.Join(ucache, fmt.Sprintf("ionc-go-%s-%s-%s", dlgoVersion, goos, goarch))
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "ionc-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debMetadata struct {
	...
	// go-ionchain version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "IonChain Builds <fjl@ethereum.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "ionc.exe" {
			ioncTool = file
		} else {
	...
	// first section contains the ionc binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"ionc":     ioncTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.ionc.nsi", filepath.Join(*workdir, "ionc.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("ionc-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "ionc.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/ionchain/ionchain-core/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("ionc.aar", filepath.Join(GOBIN, "ionc.aar"))
		os.Rename("ionc-sources.jar", filepath.Join(GOBIN, "ionc-sources.jar"))
		return
	}
	...
	archive := "ionc-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("ionc.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/ionchain/ionchain-core/mobile")
	...
	archive := "ionc-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Ionc.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Ionc.podspec", "--allow-warnings")
	}
}
