...
type ChainIndexer struct {
	chainDb  ioncdb.Database     // Chain database to index the data from
	indexDb  ioncdb.Database     // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb ioncdb.Database, indexDb ioncdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
