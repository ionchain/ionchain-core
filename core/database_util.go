...
var (
	headHeaderKey = []byte("LastHeader")  //存储区块链中最新生成的区块的区块头的哈希值
...
//获取区块链中最新生成的区块的区块头的哈希值，这个估计是给轻节点用的有待考证
...
//当采用快速同并方式的时候，获取当前区块链中最新区块的区块头的哈希值
...
//根据区块的哈希值获取区块头的RLP值（一中二进制编码的数据结构）
...
// found.
//根据区块编号和区块链哈希值获取区块头的详细信息，根据代码似乎是通过reflect自动映射字段，不需要在编码和解码的部分进行修改
...
// none found.
//获取区块中包含的难度值， 这个难度值应该是总的难度值，而不是区块头中的区块难度值
