...
var (
	...
	configPrefix   = []byte("ionchain-config-") // config prefix for the db
...
func WriteCanonicalHash(db ioncdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db ioncdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db ioncdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db ioncdb.Putter, hash common.Hash) error {
...
func WriteHeader(db ioncdb.Putter, header *types.Header) error {
...
func WriteBody(db ioncdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db ioncdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db ioncdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db ioncdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db ioncdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db ioncdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db ioncdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db ioncdb.Database) ioncdb.Database {
	return ioncdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db ioncdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db ioncdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db ioncdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
