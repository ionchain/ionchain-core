...
func loadAndParseLegacyJournal(db ioncdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
...
func loadAndParseJournal(db ioncdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Ionc
	...
	// It can happen that Ionc crashes without persisting the latest
	// diff journal.
...
func loadSnapshot(diskdb ioncdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, error) {
	...
	//
	// Possible scenario: Ionc was crashed without persisting journal and then
