...
func ReadSnapshotRoot(db ioncdb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db ioncdb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db ioncdb.KeyValueWriter) {
...
func ReadAccountSnapshot(db ioncdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db ioncdb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db ioncdb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db ioncdb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db ioncdb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db ioncdb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db ioncdb.Iteratee, accountHash common.Hash) ioncdb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db ioncdb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db ioncdb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db ioncdb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db ioncdb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db ioncdb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db ioncdb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db ioncdb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db ioncdb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db ioncdb.KeyValueWriter) {
