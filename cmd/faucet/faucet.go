...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the IonChain protocol")
	statsFlag   = flag.String("ioncstats", "", "Ethstats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for IonChain and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // IonChain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an IonChain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // IonChain protocol stack
	client *ioncclient.Client  // Client connection to the IonChain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the IonChain light client protocol
	cfg := ionc.DefaultConfig
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the IonChain service: %w", err)
	}

	// Assemble the ioncstats monitoring and reporting service'
	if stats != "" {
		if err := ioncstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := ioncclient.NewClient(api)
...
// close terminates the IonChain connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the IonChain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and IonChain address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// and scrape it for the IonChain address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No IonChain address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and IonChain address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the IonChain address and profile URL.
	res, err := http.Get(url)
	if err != nil {
		return "", "", common.Address{}, err
	}
...
// authNoAuth tries to interpret a faucet request as a plain IonChain address,
