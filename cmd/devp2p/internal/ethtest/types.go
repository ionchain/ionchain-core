...
import (
	...
	"github.com/ionchain/ionchain-core/internal/utesting"
	"github.com/ionchain/ionchain-core/p2p"
	"github.com/ionchain/ionchain-core/p2p/rlpx"
...
// Status is the network packet for the status message for ionc/64 and later.
type Status struct {
...
func (c *Conn) handshake(t *utesting.T) Message {
	...
	ourHandshake := &Hello{
		Version: 5,
		Caps: []p2p.Cap{
			{Name: "ionc", Version: 64},
			{Name: "ionc", Version: 65},
		},
		ID: pub0,
	}
	...
	case *Hello:
		...
		if c.ethProtocolVersion == 0 {
			t.Fatalf("unexpected ionc protocol version")
		}
		return msg
	default:
...
// negotiateEthProtocol sets the Conn's ionc protocol version
// to highest advertised capability from peer
func (c *Conn) negotiateEthProtocol(caps []p2p.Cap) {
	var highestEthVersion uint
	for _, capability := range caps {
		if capability.Name != "ionc" {
			continue
		}
...
loop:
	...
	// make sure ionc protocol version is set for negotiation
	if c.ethProtocolVersion == 0 {
		t.Fatalf("ionc protocol version must be set in Conn")
	}
	if status == nil {
