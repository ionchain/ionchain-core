...
import (
	...
	"github.com/ionchain/ionchain-core/contracts/checkpointoracle"
...
func newClient(ctx *cli.Context) *ioncclient.Client {
	client, err := ioncclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to IonChain node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, ioncclient.NewClient(client))
	if err != nil {
