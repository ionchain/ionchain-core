...
import (
	...
	"github.com/ionchain/ionchain-core/console/prompt"
...
const (
	clientIdentifier = "ionc" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-ionchain command line interface")
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.LegacyTestnetFlag.Name):
		log.Info("Starting Ionc on Ropsten testnet...")
	...
	case ctx.GlobalIsSet(utils.RinkebyFlag.Name):
		log.Info("Starting Ionc on Rinkeby testnet...")

	case ctx.GlobalIsSet(utils.GoerliFlag.Name):
		log.Info("Starting Ionc on Görli testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Ionc in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Ionc on IonChain mainnet...")
	}
...
func startNode(ctx *cli.Context, stack *node.Node, backend ioncapi.Backend) {
	...
	// Create a client to interact with local ionc node.
	rpcClient, err := stack.Attach()
	if err != nil {
		utils.Fatalf("Failed to attach to self: %v", err)
	}
	ethClient := ioncclient.NewClient(rpcClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		ethBackend, ok := backend.(*ionc.EthAPIBackend)
		if !ok {
			utils.Fatalf("IonChain service not running: %v", err)
		}
