...
import (
	...
	"github.com/ionchain/ionchain-core/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute ionchain requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var ionc = web3.ionc; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ionc.getBalance(ionc.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current ionc instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic ionc metadata
	fmt.Fprintf(c.printer, "Welcome to the ionc JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + ionc.coinbase);
		console.log("at block: " + ionc.blockNumber + " (" + new Date(1000 * ionc.getBlock(ionc.blockNumber).timestamp) + ")");
