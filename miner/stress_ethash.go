...
func makeMiner(genesis *core.Genesis) (*node.Node, *ionc.IonChain, error) {
	// Define the basic configurations for the IonChain node
	...
	// Create the node and configure a full IonChain node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, nil, err
	}
	ethBackend, err := ionc.New(stack, &ionc.Config{
		Genesis:         genesis,
		...
		GPO:             ionc.DefaultConfig.GPO,
		Ethash:          ionc.DefaultConfig.Ethash,
		Miner: miner.Config{
