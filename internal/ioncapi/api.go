...
func NewPublicIONChainAPI(b Backend) *PublicIONChainAPI {
	return &PublicIONChainAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicIONChainAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current ionchain protocol version this node supports
func (s *PublicIONChainAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicIONChainAPI) Syncing() (interface{}, error) {
...
// 发送交易，该方法的参数是原始参数，需要在方法内签名，
// 防止从console中发送unlock的账户
...
//   keccak256("\x19ionchain Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19ionchain Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an ionchain ECDSA signature for:
// keccack256("\x19ionchain Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/ionchain/ionchain-core/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19ionchain Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/ionchain/ionchain-core/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid ionchain signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the ionchain blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new ionchain blockchain API.
...
//
// https://github.com/ionchain/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of ionchain APIs exposed over the public
// debugging endpoint.
...
// Version returns the current ionchain protocol version.
