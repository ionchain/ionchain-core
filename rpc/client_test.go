...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("ionc", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("ionc", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-ionchain-test-ipc-%d-%d", os.Getpid(), rand.Int63())
