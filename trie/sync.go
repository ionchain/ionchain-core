...
type Sync struct {
	database ioncdb.KeyValueReader    // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database ioncdb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
// both ionc/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw ioncdb.Batch) error {
